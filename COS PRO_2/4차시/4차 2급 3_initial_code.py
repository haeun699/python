...
def func_c(bundle): # 카드의 점수 구하기 함수
    ...
    score_per_cards = { # 딕셔너리
        ...
        'd': 4,
        'e': 5
    }                   # a는 1점 b는 2점 c는 3점 d는 4점 e는 5점
    for card in bundle:
        answer += score_per_cards[card] # 카드를 넣어서 점수를 호출해서 함수 더하기
    return answer
        
def solution(n, bundle): # n : 서로 번갈아 가면서 뽑는 카드 수 bundle : 카드 뭉치
    a_cards = func_a( bundle , 0 )[:n] # 0부터 끝까지
    b_cards = func_a( bundle , 1 )[:n] # 1부터 끝까지
